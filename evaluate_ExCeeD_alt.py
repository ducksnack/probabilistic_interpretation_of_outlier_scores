...
def compute_confidence_error(model, Dtrain, y, splits = 5, iterations = 1000, saveresults = True):
    ...
    l2_error = {'ExCeeD': np.zeros(2, np.float),'ExCeeD_cant': np.zeros(2, np.float), 'ExCeeD_sp': np.zeros(2, np.float),
                'Squash': np.zeros(2, np.float),
    ...
    for train_index, test_index in skf.split(Dtrain, y):
        ...
        exceed_conf, exceed_conf_cant, exceed_conf_otherprior, squash_conf, linear_conf, unify_conf, logcal_conf, isocal_conf,\
        betacal_conf, prediction = confidence_estimation(X_train, y_train, X_test, y_test, contamination, model)
        ...
        if saveresults:
            savedf_results(exceed_conf, exceed_conf_cant, exceed_conf_otherprior, squash_conf, linear_conf, unify_conf, logcal_conf,
                           isocal_conf, betacal_conf, frequence, prediction, y_test, idx_crossval)
        ...
        l2_error['ExCeeD_cant'] += [l2_distance(exceed_conf_cant[normals], frequence[normals])/splits,
                              l2_distance(exceed_conf_cant[anomalies], frequence[anomalies])/splits]
...
def confidence_estimation(X_train, y_train, X_test, y_test, contamination, model):
    ...
    exceed_conf_cant: example-wise confidence using ExCeeD (outlier probability computed bt using the cantelli inequality), our method
    ...
    #prob_func_cant = np.vectorize(cantelli_inequality)
    #exceed_posterior_cant = prob_func_cant(test_scores, score_mean, score_var)
    exceed_posterior_cant = [cantelli_inequality(s, score_mean, score_var) for s in test_scores]
    ...
    exceed_conf_cant = conf_func(exceed_posterior_cant)
    np.place(exceed_conf_cant, prediction == 0, 1 - exceed_conf_cant[prediction == 0])
    ...
    return exceed_conf, exceed_conf_cant, exceed_conf_sp, squash_conf, linear_conf, unify_conf, logcal_conf, isocal_conf, betacal_conf, prediction
...
def savedf_results(exceed_conf, exceed_conf_cant, exceed_conf_otherprior, squash_conf, linear_conf, unify_conf, logcal_conf, isocal_conf,
    ...
    results['ExCeeD_cant'] = exceed_conf_cant
